# Workspace & data model

A Taskunity workspace is just a folder. Everything is a plain file you own, can diff, and can
commit to git.

```text
workspace/
  config.json           # workspace/app metadata
  projects/
    apollo.json         # one file per project
  tasks/
    A1B2-C3D4-E5F6-7890.json  # one file per task (native id format)
  milestones/
    M-1A2B-3C4D.json    # one file per milestone
  assets/
    A1B2-C3D4-E5F6-7890/  # attachments live alongside their task id
      screenshot.png
```

## Workspace config (`config.json`)

Each workspace has a root-level `config.json` with basic editable metadata used by the UI.

```json
{
  "app_name": "Taskunity",
  "workspace_name": "My Workspace",
  "workspace_description": "Local file-backed workspace/task tracker",
  "export_title": "My Workspace"
}
```

## Project files (`projects/*.json`)

Each project is stored as one JSON file under `projects/` (for example, `projects/apollo.json`).
Projects provide the color and description used across the board, timeline, and milestone views.

```json
{
  "name": "Apollo",
  "description": "First initiative",
  "color": "#d82cd3"
}
```

## Task files

Each task is one JSON file in `tasks/`. Native task ids use the `XXXX-XXXX-XXXX-XXXX` uppercase
hex format generated by the UI.

```json
{
  "id": "A1B2-C3D4-E5F6-7890",
  "title": "CAF refinement prototype",
  "status": "working",
  "priority": "high",
  "project": "Apollo",
  "summary": "Build a prototype CAF refinement workflow.",
  "description": "Compare baseline sharpness against protection-aware refinement.",
  "tags": ["caf", "prototype"],
  "start_date": "2026-06-24",
  "due_date": "2026-06-30",
  "completed_date": null,
  "percent_complete": 60,
  "depends_on": ["0EBB-528F-371E-61AE"],
  "checklist": [
    {"text": "Generate baseline", "done": true},
    {"text": "Export plots", "done": false}
  ],
  "activity": [
    {
      "id": "a1b2c3d4",
      "event_type": "note",
      "created_at": "2026-06-26T10:30:00",
      "note_text": "Progress note."
    },
    {
      "id": "e5f6a7b8",
      "event_type": "progress_update",
      "created_at": "2026-06-26T11:00:00",
      "progress_before": 40,
      "progress_after": 60
    }
  ]
}
```

### Field reference

| Field | Type | Notes |
| --- | --- | --- |
| `id` | string | Unique; matches the filename |
| `title` | string | Display name |
| `status` | string | e.g. `backlog`, `working`, `blocked`, `done` |
| `priority` | string | e.g. `low`, `medium`, `high` |
| `project` | string | Project name from a file in `projects/` |
| `summary` | string | Short one-liner |
| `description` | string | Longer description |
| `tags` | string[] | Free-form labels |
| `start_date` / `due_date` | date | `YYYY-MM-DD`; used by the timeline and calendar |
| `completed_date` | date or null | Set when finished |
| `percent_complete` | int | 0–100, drives progress bars |
| `depends_on` | string[] | Other task **ids** this task depends on |
| `checklist` | object[] | `{text, done}` items |
| `activity` | object[] | Unified activity log (notes, images, progress changes) |

### Activity log events

Each entry in the `activity` array is a `TaskActivityEvent`:

| Field | Type | Notes |
| --- | --- | --- |
| `id` | string | Random hex identifier |
| `event_type` | string | `note`, `image`, or `progress_update` |
| `created_at` | datetime | ISO 8601 timestamp, set automatically |
| `note_text` | string or null | Text content for `note` events |
| `image_path` | string or null | Relative path for `image` events |
| `image_filename` | string or null | Original filename for `image` events |
| `progress_before` | int or null | Previous percent for `progress_update` events |
| `progress_after` | int or null | New percent for `progress_update` events |

### Dependencies

`depends_on` stores task **ids**. In the UI you add dependencies with a search-as-you-type picker
that finds tasks by name and shows their status, project, due date, and id — then stores the id.
Dependencies appear on the Gantt timeline as an `↳ after <name>` label plus a marker where the
dependency's bar ends.

## Attachments / assets

Uploaded images and files are stored under `assets/<TASK-ID>/` and referenced from the task's
`activity` log. Removing a task's activity entries is as simple as editing the JSON (or using
the panel).

## Milestone files

Each milestone is one JSON file in `milestones/`. Milestones group tasks across projects and have
their own notes and attachments. Milestone ids look like `M-XXXX-XXXX`.

```json
{
  "id": "M-1A2B-3C4D",
  "title": "Beta launch",
  "status": "active",
  "color": "#3567e0",
  "summary": "First public beta.",
  "description": "Cross-project work required to ship the beta.",
  "projects": ["Apollo", "Gemini"],
  "start_date": "2026-06-24",
  "target_date": "2026-07-15",
  "task_ids": ["A1B2-C3D4-E5F6-7890", "0EBB-528F-371E-61AE"],
  "notes": [],
  "attachments": [],
  "extra": {}
}
```

| Field | Type | Notes |
| --- | --- | --- |
| `id` | string | Unique; matches the filename |
| `title` | string | Display name |
| `status` | string | `planned`, `active`, or `done` |
| `color` | string | Hex colour used for milestone card/banner styling |
| `summary` | string | Short one-liner |
| `description` | string | Longer description |
| `projects` | string[] | Project names this milestone spans |
| `start_date` / `target_date` | date | `YYYY-MM-DD` |
| `task_ids` | string[] | Task ids belonging to the milestone |
| `notes` | object[] | `{created_at, body}` entries |
| `attachments` | object[] | `{filename, path, kind, description, uploaded_at}` |

A task may appear in any number of milestones, and milestones never duplicate task data — they only
reference task ids. See {doc}`milestones` for how to use them.
</content>

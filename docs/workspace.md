...
`programs/default.json`). A program file holds the program-level spec and the list of projects.
Each project has a `name`, optional `description`, and a `color` used to tint board cards and
timeline labels.
...
| `project` | string | Project name from the active file in `programs/` |
